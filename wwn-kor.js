...
Hooks.once("ready", async () => {
  ...
  const currentVersion = game.settings.get("wwn-kor", "systemMigrationVersion");
  const NEEDS_MIGRATION_VERSION = "1.0.0";
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;
  if ( !currentVersion && totalDocuments === 0 ) return game.settings.set("wwn-kor", "systemMigrationVersion", game.system.version);
